      ...
      {filteredOrders.length === 0 ? (
        ...
        filteredOrders.map((order) => (
          <div key={order._id} className="mb-4">
            ...
            >
              <div className="head-left">
                <div className="detail1 flex justify-between md:justify-start items-center">
                  <p
                    className={`${getStatusColor(order.status)} py-1 px-4 rounded-3xl min-w-25 text-sm flex items-center justify-center text-center font-medium`}
                  >
                    {order.status}
                  </p>
                  ...
                  <div
                    className={`ml-auto text-xl transition-transform duration-300 ${expandedOrderId === order._id ? "rotate-90 text-[#ff74b1]" : ""}`}
                  >
                    {">"}
                  </div>
                </div>
              ...
              <div className="card-body flex flex-row items-center justify-between mt-4 md:mt-0">
                <div className="card-info flex justify-start items-center gap-3 md:gap-4">
                  ...
                  <div className="detail2">
                    ...
                    <p className="text-sm text-[#ff74b1] font-bold">
            ...
            <div
              className={`overflow-hidden transition-all duration-300 bg-white mx-1 rounded-b-xl border-x border-b 
        ${expandedOrderId === order._id ? "max-h-250 p-4 opacity-100 border-gray-300" : "max-h-0 opacity-0 border-transparent"}`}
            >
              <h4 className="text-sm font-bold mb-3 text-gray-500 uppercase tracking-wide">
                Order Items:
              </h4>
              ...
              <div className="pt-3 border-t border-gray-300 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-[#ff74b1]">
                  {order.total_price} THB
                </span>
              </div>
            </div>
